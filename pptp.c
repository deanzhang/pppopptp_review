...
static int open_callmgr(int call_id, struct in_addr inetaddr, char *phonenr,
			int window);
static void launch_callmgr(int call_is, struct in_addr inetaddr, char *phonenr,
			   int window);
...
struct channel pptp_channel = {	//见pppd.h中的定义
	...
	//process_extra_options: &PPPOEDeviceOptions,
      ...
      establish_ppp:&generic_establish_ppp,
				//此函数原型定义于pppd源代码中的sys-linux.c中，用于将一个文件描述符fd转换为ppp接口
      disestablish_ppp:&generic_disestablish_ppp,
					//此函数原型定义于pppd源代码中的sys-linux.c中
...
static int pptp_start_client(void)
{
	...
	if (setsockopt
	    (pptp_fd, 0, PPTP_SO_TIMEOUT, &pptp_timeout, sizeof(pptp_timeout)))
	...
	do {
		 ...
		 */
		callmgr_sock =
		    open_callmgr(src_addr.sa_addr.pptp.call_id,
				 dst_addr.sa_addr.pptp.sin_addr, pptp_phone,
				 50);
		/* Exchange PIDs, get call ID */
	} while (get_call_id
		 (callmgr_sock, getpid(), getpid(),
		  &dst_addr.sa_addr.pptp.call_id) < 0);
			...
			int window)
{
	...
	if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
		fatal("Could not create unix domain socket: %s",
		      strerror(errno));
	}
	/* Make address */
			...
			case 0:	/* child */
				{
					...
					// close(pty_fd);
					//close(gre_fd);
					launch_callmgr(call_id, inetaddr,
						       phonenr, window);
				}
			default:	/* parent */
...
static void launch_callmgr(int call_id, struct in_addr inetaddr, char *phonenr,
			   int window)
{
	char win[10];
	char call[10];
	char *my_argv[9] =
	    { "pptp", inet_ntoa(inetaddr), "--call_id", call, "--phone",
     phonenr, "--window", win, NULL };
	char buf[128];
